<template>
	<div id="signal-edit">
		...
		<signal-dev-tabs :activeTab="'edit-strategy'"></signal-dev-tabs>
		<el-row class="edit-strategy-pane">
			<el-col :span="IDECols" :class="{'hide-pane': shouldHideIDEPane}">
				<el-row class="IDE-toolbar">
					<el-col :span="12">
						...
						<el-button class="toolbar-icon toolbar-icon-link"><router-link to="/SignalDevelop/SignalEdit/funcLib" style="text-decoration: none;">函数库</router-link></el-button>
					</el-col>
					<el-col :span="8" :offset="2">
						<div style="float: right;">
							<el-button class="toolbar-icon toolbar-icon-link"><router-link to="/SignalDevelop/SignalEdit/strategyAPI" style="text-decoration: none;">API</router-link></el-button>
				...
				<el-row>
					<div>
						...
						<!-- 备注一下，以后要是再获取不到code就去node_model中找vue-ace改一下$emit前面的那个删了 -->
			...
			<el-col :span="11" :class="{'hide-pane': shouldHideBuildPane}" style="width: 49%;">
					...
					<div :class="{'hide-pane': shouldHideBuildToolbar}">
						<el-col :span="20">
							<div>
								...
								<span>￥</span>
								<!--将 v-model="amountMoney" 改为 v-model="bought",  这里是庚鑫改动过的 -->
								<el-input v-model="bought" style="width: 80px;"></el-input>
...
export default {
	...
	methods: {
		...
		getLogHeight: function(_windowHeight) {
			...
			console.log("****Log　height" + ((_editorHeight - IDEHeight - logToolbarHeight) > 0 ? (_editorHeight - IDEHeight - logToolbarHeight) : 0));
			...
			this.$http.post(Global.getRequestUrl() + "/dev/signal/compile", 
				...
				if(res.success) {
					var stateIntervalID = setInterval(function() {
						...
						self.$http.post(Global.getRequestUrl() + "/dev/signal/test_result",
							...
							function(stateRes) {
								 if(stateRes.data && stateRes.data.data && stateRes.data.data.report && stateRes.data.data.report.final_status) {
								 	...
								 	if(stateRes.data.data.report.final_status === "fail") {
								 		console.error("编译运行出错！错误原因：" + stateRes.data.data.report.reason);
		...
		runBackTest: function() {
			...
			// 庚鑫在这又做了改动，再传到后台的数据中添加了一个bought
			var params = {
				...
				task_type: this.getStrategyCatVal(this.$route.params.strategyCat),
				startTime: new Date(this.startDate).getTime()/1000,
				endTime: new Date(this.endDate).getTime()/1000,
				frequency: parseInt(this.frequency)
			};

			// console.log(params);
			this.$http.post(Global.getRequestUrl() + "/dev/signal/run_test", 
				...
				// resp = resp.data;
				if(resp.success) {
					var backtestId = resp.data.name;
					this.$router.push("/SignalDevelop/BackTestDetail/" + signalId + "/" +backtestId);
					console.log("运行回测成功！");
				}else {
					console.error("运行回测失败！error code: " + resp.code + ", reason: " + resp.msg);
			...
			this.$http.post(Global.getRequestUrl() + "/dev/signal/update", 
				{
					...
					name: window.sessionStorage.getItem("signalName"),
